...
# Re-fit an identical model on plain numpy arrays (no column names) since
# onnxmltools' XGBoost converter breaks on string feature names like "amount".
import xgboost as xgb
X_all = df[features].values.astype(np.float32)
y_all = df["is_fraud"].values
model_np = xgb.XGBClassifier(**model.get_params())
model_np.fit(X_all, y_all)
model = model_np  # use this numpy-trained version for both latency + onnx export below
...
print("\nSaved ONNX model to fraud_model.onnx")
...
# Sanity check: outputs should roughly match
