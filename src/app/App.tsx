...
import {
  ...
  IndianRupee, Loader2, Radio, Lock, Unlock, Flag, Info,
} from "lucide-react";
import {
...
// SEED_TRANSACTIONS / SEED_ALERTS: fallback demo data, only shown if the
// backend at localhost:8000 is unreachable (e.g. `npm run dev` without the
// API running). When the backend IS running, `useLiveTransactions()` below
// replaces this with real transactions from /recent. Kept as `TRANSACTIONS`
// / `ALERTS` names purely so existing `typeof TRANSACTIONS[0]` TYPE
// annotations elsewhere in this file keep working unchanged.
const TRANSACTIONS = [
...
// Maps a raw row from GET /recent (or /transactions) into the same shape
// as TRANSACTIONS above, so every screen that already renders TRANSACTIONS
// fields works unchanged with real data.
function mapBackendRow(row: any): typeof TRANSACTIONS[0] {
  const score = Math.round(100 - (row.risk_score ?? 0)); // backend: higher=safer -> UI: higher=riskier
  const status =
    row.decision === "BLOCK" ? "Blocked" :
    row.decision === "VERIFY" ? "Flagged" : "Allowed";
  const d = row.timestamp ? new Date(row.timestamp.replace(" ", "T")) : new Date();
  return {
    id: "TXN-" + row.id,
    sender: row.sender_name || "Unknown Sender",
    sUPI: row.sender_vpa || "unknown@upi",
    receiver: row.receiver_name || "Unknown Receiver",
    rUPI: row.receiver_vpa || "unknown@upi",
    amount: row.amount,
    time: isNaN(d.getTime()) ? "--:--:--" : d.toLocaleTimeString("en-IN", { hour12: false }),
    score,
    status,
    type: row.transaction_type || (score >= 75 ? "High Risk" : "Normal"),
  };
}
...
// ─── TOP NAV ──────────────────────────────────────────────────────────────────
function TopNav({ page, onRefresh }: { page: string; onRefresh: () => void }) {
  ...
  return (
    <header className="h-14 bg-white dark:bg-[#1e293b] border-b border-slate-200 dark:border-white/[0.07] flex items-center px-6 gap-4 shrink-0 transition-colors duration-200">
      <div className="flex items-center gap-1.5 text-sm">
        <span className="text-slate-400 dark:text-slate-500 text-xs">SentinelAI</span>
        <ChevronRight size={12} className="text-slate-300 dark:text-slate-600" />
        <span className="font-semibold text-slate-700 dark:text-slate-100">{titles[page]}</span>
      </div>

      <div className="flex-1 max-w-xs ml-4">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-white/[0.07] rounded-lg">
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <button onClick={toggle}
          ...
          <span className="text-[11px] font-medium text-slate-600 dark:text-slate-300 hidden md:block">{isDark ? "Light" : "Dark"}</span>
        </button>
  ...
  const handlePay = () => {
    ...
    const iv = setInterval(() => {
      ...
      if (step >= scanSteps.length - 1) {
        ...
        // Build the request payload the backend's /evaluate endpoint expects.
        // The form only collects receiver + amount, so the remaining fields
        // are sensible defaults/derivations for demo purposes — a real app
        // would pull these from the device SDK, session history, etc.
        ...
        const payload = {
          ...
          amount: amt,
          new_beneficiary: !matchedReceiver,
          unusual_location: false,
          late_night: hour >= 23 || hour < 5,
        ...
        fetch(`${API_URL}/evaluate`, {
          ...
          .then((data) => {
            // Backend's risk_score is 0-100 where HIGHER = SAFER (matches
            // risk_engine.py: final_score >= 80 -> LOW risk -> ALLOW).
            // The dashboard's score convention is the opposite (higher =
            // riskier, drives the red/amber/green bars), so invert it here.
            const displayScore = Math.round(100 - data.risk_score);
            const status: TxStatus =
              data.decision === "BLOCK" ? "Blocked" :
              data.decision === "VERIFY" ? "Flagged" : "Allowed";
            const txnId = "TXN-" + (data.transaction_id ?? Math.floor(800000 + Math.random() * 99999));

            setResult({ status, score: displayScore, latency: "~1ms", txnId });
            setPayState("result");
          })
          .catch((err) => {
            ...
            <button
              onClick={() => {
                ...
                const synth = {
                  ...
                  score: result.score,
                  type: result.status === "Blocked" ? "High Risk" : result.status === "Flagged" ? "Flagged" : "Normal",
                };
    ...
    <div className="flex items-start justify-center gap-5">
      <Card className="w-full max-w-md p-6">
        ...
        <button onClick={handlePay} disabled={!receiver || !amount}
          className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-600/25">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Live Transaction Feed</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500">Click any row to open Transaction Analysis</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="xl:col-span-1">
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 dark:border-white/[0.06]">
            ...
            <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Click to analyse</p>
          </div>
          <div className="divide-y divide-slate-50 dark:divide-white/[0.04] max-h-[600px] overflow-y-auto">
            {TRANSACTIONS.map(tx => (
...
function MulePage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Mule Network Analysis</h2>
...
function AlertsPage({ onAnalyse }: { onAnalyse: (t: typeof TRANSACTIONS[0]) => void }) {
  ...
  const activeAlerts = TRANSACTIONS
    .filter(t => t.status !== "Allowed")
    .map(t => ({
      id: "ALT-" + t.id.replace("TXN-", ""),
      type: t.type,
      amount: t.amount,
      sender: t.sender,
      sUPI: t.sUPI,
      ...
      <div className={`${isDark ? "dark" : ""} flex h-screen w-full overflow-hidden`}
        ...
        <div className="flex h-full w-full bg-[#f0f4fa] dark:bg-[#0f172a] transition-colors duration-200">
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            <TopNav page={page} onRefresh={() => setRefresh(r => r + 1)} />
            <main className="flex-1 overflow-y-auto p-5 sentinel-scroll">
              {pages[page]}
            </main>
          </div>
        </div>
